...
const versionInfoURL = 'https://gitlab.com/Sczr0/Phigros-upd/-/raw/main/version_info.json?ref_type=heads';
const appendixURL = 'https://gitlab.com/Sczr0/Phigros-upd/-/raw/main/appendix.txt?ref_type=heads';
const constantsFileURL = 'https://gitlab.com/Sczr0/Phigros-upd/-/raw/main/song.txt?ref_type=heads'; // 固定文件名改为 song.txt
const readmeURL = 'https://raw.githubusercontent.com/Catrong/phi-plugin/main/README.md'; // GitHub README 的原始链接

// 获取文件内容并解析版本号
async function getFileVersionAndContent(fileUrl) {
  try {
...
export class PhigrosUpdatePlugin extends plugin {
  ...
  async handlePhigrosUpdateCommand(e) {
    try {
      ...
      // 获取附言内容
      const appendix = await this.getAppendix();

      // 判断时间并选择曲目定数文件
      const isAfterFivePM = this.checkIfAfterFivePM();
      const { version: constantsFileVersion, content: songConstantsContent } = await getFileVersionAndContent(constantsFileURL);
      ...
      if (this.isUpdateDay() && !isAfterFivePM) {
        response += `\n注意: 当前时间尚未过下午5点，更新的新曲定数可能尚未发布。`;
      }

      // 发送响应
      await e.reply(response, false, { at: false });
    } catch (error) {
